...
export class ExportController {
  ...
  async exportTasks(
    ...
    try {
      ...
      logger.error('Error exporting tasks', error as any);
      next(error);
    }
  }
  ...
  async exportAnalytics(
    ...
    try {
      ...
      if (format === 'csv') {
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="analytics-${type}-${timestamp}.csv"`
        );
        res.send(csvData);
      } else {
        // Return as JSON
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="analytics-${type}-${timestamp}.json"`
        );
        res.json({
          data: analyticsData,
          metadata: {
      ...
      logger.error('Error exporting analytics', error as any);
      next(error);
    }
  }
  ...
  async getExportProgress(
    ...
    try {
      ...
      const { asyncExportService } = await import(
        '../services/asyncExportService'
      );
  ...
  async downloadExport(
    ...
    try {
      ...
      logger.error('Error downloading export', error as any);
      next(error);
    }
  }
