...
export function useTaskUpdates(options: UseTaskUpdatesOptions = {}): UseTaskUpdatesReturn {
  const { projectId, onUpdateSuccess, onUpdateError, optimisticUpdates = true } = options
  ...
  const applyOptimisticUpdate = useCallback(
    (boardData: TaskBoardData, taskId: number, updates: Partial<Task>): TaskBoardData => {
      ...
      if (updates.status) {
        const sourceColumn = newColumns.find((col) => col.tasks.some((task) => task.id === taskId))
   ...
   */
  const revertOptimisticUpdate = useCallback((originalBoardData: TaskBoardData): TaskBoardData => {
  ...
  const updateTaskStatus = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          error instanceof ApiError ? error.message : 'Failed to update task status'
