...
export const SettingsProvider = ({ children, autoLoad = true }: SettingsProviderProps) => {
  ...
  const updateCategory = async (category: SettingsCategory, data: any) => {
    try {
      ...
      dispatch({
        type: 'SET_ERROR',
        payload: error instanceof Error ? error.message : `Failed to update ${category} settings`,
      })
      throw error // Re-throw to allow component-level error handling
    }
  }
...
export function useOptimisticSettings() {
  ...
  const [optimisticState, setOptimisticState] = React.useState<Partial<UserSettings> | null>(null)
