...
import { commandService, type CommandRequest, type CommandResult, type CommandSequenceRequest } from '../services/commandService';
...
export interface UseCommandExecutionReturn {
  ...
  executePreset: (presetName: string, workingDirectory?: string, repositoryPath?: string) => Promise<void>;
  clearResult: () => void;
}
...
export function useCommandExecution(): UseCommandExecutionReturn {
  ...
  const executeCommand = useCallback(async (request: CommandRequest) => {
    setState(prev => ({
      ...prev,
      isExecuting: true,
      error: null,
    ...
    try {
      showInfo('Command Execution', `Executing: ${request.command} ${(request.args || []).join(' ')}`);
  ...
  const executeSequence = useCallback(async (request: CommandSequenceRequest) => {
    setState(prev => ({
      ...prev,
      isExecuting: true,
      error: null,
    ...
    try {
      const commandNames = request.commands.map(cmd => 
      ...
      if (response.success && response.data) {
        ...
        if (successCount === totalCount) {
          showSuccess(
            'Sequence Completed',
            `All ${totalCount} commands executed successfully`
          );
        } else {
          showError(
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to execute command sequence';
      setState(prev => ({
        ...prev,
        isExecuting: false,
        error: errorMessage,
  ...
  const executePreset = useCallback(async (presetName: string, workingDirectory?: string, repositoryPath?: string) => {
    try {
