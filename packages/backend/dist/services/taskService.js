...
class TaskService {
        ...
        try {
            ...
            const task = await prisma.task.create({
                data: {
                    ...
                    status: data.status || prisma_1.TaskStatus.PENDING,
                    priority: data.priority || prisma_1.Priority.MEDIUM,
                    order: data.order || 0,
    ...
    async getTaskCountsByStatus(projectId) {
        try {
            ...
            const [total, pending, inProgress, completed, blocked, cancelled] = await Promise.all([
                ...
                prisma.task.count({ where: { projectId, status: prisma_1.TaskStatus.PENDING } }),
                prisma.task.count({ where: { projectId, status: prisma_1.TaskStatus.IN_PROGRESS } }),
                prisma.task.count({ where: { projectId, status: prisma_1.TaskStatus.COMPLETED } }),
                prisma.task.count({ where: { projectId, status: prisma_1.TaskStatus.BLOCKED } }),
                prisma.task.count({ where: { projectId, status: prisma_1.TaskStatus.CANCELLED } })
            ]);
            return {
                total,
                pending,
        ...
        try {
            ...
            const tasks = await prisma.task.findMany({
                where: {
                    ...
                    status: {
                        not: prisma_1.TaskStatus.COMPLETED
                    }
                },
                orderBy: {
                    dueDate: 'asc'
    ...
    async bulkUpdateTaskStatus(taskIds, status) {
        try {
            ...
            if (!Object.values(prisma_1.TaskStatus).includes(status)) {
        ...
        try {
            ...
            return await this.dbService.transaction(async (prisma) => {
                const taskData = tasks.map(task => ({
                    ...
                    status: task.status || prisma_1.TaskStatus.PENDING,
                    priority: task.priority || prisma_1.Priority.MEDIUM,
                    order: task.order || 0,
    ...
    async getTaskStatistics(projectId) {
        try {
            ...
            const [highPriority, mediumPriority, lowPriority, urgentPriority] = await Promise.all([
                prisma.task.count({ where: { projectId, priority: prisma_1.Priority.HIGH } }),
                prisma.task.count({ where: { projectId, priority: prisma_1.Priority.MEDIUM } }),
                prisma.task.count({ where: { projectId, priority: prisma_1.Priority.LOW } }),
                prisma.task.count({ where: { projectId, priority: prisma_1.Priority.URGENT } })
            ]);
