...
export class TaskService {
  ...
  async loadTasksFromProject(projectId: string): Promise<TasksData> {
    ...
    try {
      ...
      throw new ApiError('NETWORK_ERROR', 'Failed to load tasks from server', 500)
    }
  }
  ...
  async loadSampleTasks(): Promise<TasksData> {
    try {
      ...
      throw new ApiError('SAMPLE_DATA_ERROR', 'Failed to load sample tasks', 500)
    }
  }
  ...
  getTaskDependents(tasksData: TasksData, taskId: number): Task[] {
    return tasksData.tasks.filter((task) => task.dependencies && task.dependencies.includes(taskId))
  }
