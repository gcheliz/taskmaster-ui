...
export const RepositoryDetailsView = ({
  ...
  const { metadata, branches, isLoading, error, isRefreshing, refresh, lastFetch } =
    useRepositoryData({
      repositoryId,
      refreshInterval,
      autoFetch: true,
  ...
  const { checkoutBranch, fetchRemote, pullFromRemote, isActionLoading, currentAction } =
    ...
    <div className={`repository-details-view ${className}`}>
      ...
      <div className="repository-details-view__header">
        <div className="header-info">
          {lastFetch && (
            <span className="last-update">Last updated: {formatLastUpdate(lastFetch)}</span>
          )}
