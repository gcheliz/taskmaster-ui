...
const HealthSummaryCard: React.FC<HealthSummaryCardProps> = ({
  ...
  return (
    <div className={cn('rounded-lg border p-4 transition-all hover:shadow-md', colorClasses[color])}>
...
export const RepositoryHealthModal: React.FC<RepositoryHealthModalProps> = ({
  ...
  const { health, isLoading: healthLoading, error: healthError, refresh: refreshHealth } = useRepositoryHealth({
    repositoryId,
    autoFetch: open && !mockService,
  });

  const { statistics, isLoading: statsLoading, setPeriod } = useRepositoryStatistics({
    repositoryId,
  ...
  const effectiveRefresh = mockService ? async () => {
  ...
  const issuesBySeverity = useMemo(() => {
    ...
    const severityCounts = effectiveHealth.issues.reduce((acc, issue) => {
      acc[issue.severity] = (acc[issue.severity] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    return [
      { name: 'Critical', value: severityCounts.critical || 0, fill: '#DC2626' },
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="text-center py-6 bg-gradient-to-br from-blue-50 to-indigo-100 rounded-lg">
          <div className="flex items-center justify-center mb-4">
            <div className="relative">
              <ResponsiveContainer width={120} height={120}>
                <RadialBarChart cx="50%" cy="50%" innerRadius="60%" outerRadius="90%" data={[{ score: effectiveHealth.score, fill: effectiveHealth.score >= 70 ? '#10B981' : effectiveHealth.score >= 50 ? '#F59E0B' : '#EF4444' }]}>
          ...
          <p className="text-sm text-gray-600">Overall assessment of repository quality and maintainability</p>
        </div>
        ...
        {effectiveStats && (
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <div className="text-2xl font-bold text-blue-600">{effectiveStats.commits.total}</div>
                <div className="text-xs text-gray-500">Total Commits</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-green-600">{effectiveStats.contributors.total}</div>
                <div className="text-xs text-gray-500">Contributors</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-purple-600">{effectiveStats.files.total}</div>
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{effectiveHealth.metrics.codeQuality.complexity}</span>
              </div>
              <div className="flex justify-between">
                <span>Duplication:</span>
                <span className="font-medium">{effectiveHealth.metrics.codeQuality.duplication}%</span>
              </div>
              <div className="flex justify-between">
                <span>Maintainability:</span>
                <span className="font-medium">{effectiveHealth.metrics.codeQuality.maintainabilityIndex}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-red-50 rounded-lg p-4">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{effectiveHealth.metrics.security.vulnerabilities}</span>
              </div>
              <div className="flex justify-between">
                <span>Outdated Deps:</span>
                <span className="font-medium">{effectiveHealth.metrics.security.outdatedDependencies}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-green-50 rounded-lg p-4">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{(effectiveHealth.metrics.performance.bundleSize / 1024).toFixed(1)} KB</span>
              </div>
              <div className="flex justify-between">
                <span>Build Time:</span>
                <span className="font-medium">{effectiveHealth.metrics.performance.buildTime}ms</span>
              </div>
            </div>
          </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          <h4 className="font-medium text-gray-900">All Issues ({effectiveHealth.issues.length})</h4>
          ...
          <div className="bg-white p-4 rounded-lg border">
            ...
            <ResponsiveContainer width="100%" height={150}>
              <AreaChart data={chartData}>
                ...
                <YAxis fontSize={12} />
                <Tooltip />
                <Area type="monotone" dataKey="commits" stroke="#10B981" fill="#10B981" fillOpacity={0.6} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg border">
            ...
            <ResponsiveContainer width="100%" height={150}>
              <AreaChart data={chartData}>
                ...
                <YAxis fontSize={12} />
                <Tooltip />
                <Area type="monotone" dataKey="contributors" stroke="#8B5CF6" fill="#8B5CF6" fillOpacity={0.6} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <ModalContent size="full" className={className}>
        <ModalHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <ModalDescription className="mt-1">
                ...
                {effectiveHealth && (
                  <Badge 
                    variant={effectiveHealth.score >= 70 ? 'success' : effectiveHealth.score >= 50 ? 'warning' : 'error'} 
                    size="sm" 
                    className="ml-2"
                  >
          ...
          <div className="flex items-center space-x-1 bg-gray-100 rounded-lg p-1 mt-4">
            {[
              { key: 'overview', label: 'Overview', icon: <DashboardIcon className="w-4 h-4" /> },
              { key: 'metrics', label: 'Metrics', icon: <ChartIcon className="w-4 h-4" /> },
              { key: 'issues', label: 'Issues', icon: <WarningIcon className="w-4 h-4" /> },
              { key: 'trends', label: 'Trends', icon: <TrendIcon className="w-4 h-4" /> },
            ].map((tab) => (
              <button
                key={tab.key}
              ...
              >
                {tab.icon}
                <span>{tab.label}</span>
                {tab.key === 'issues' && effectiveHealth?.issues && effectiveHealth.issues.length > 0 && (
        ...
        <ModalFooter>
          <div className="flex items-center justify-between w-full">
            ...
            <div className="flex space-x-3">
              <Button variant="outline" onClick={effectiveRefresh} disabled={effectiveHealthLoading}>
