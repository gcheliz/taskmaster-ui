...
describe('RepositoryValidationService - Basic Tests', () => {
  ...
  beforeEach(() => {
    service = RepositoryValidationService.getInstance();
  });

  describe('getInstance', () => {
    it('should return a singleton instance', () => {
      const instance1 = RepositoryValidationService.getInstance();
      const instance2 = RepositoryValidationService.getInstance();
      
      expect(instance1).toBe(instance2);
      expect(instance1).toBeInstanceOf(RepositoryValidationService);
    });
  });
  ...
  describe('result metadata', () => {
    it('should generate unique request IDs on multiple calls', () => {
      const service1 = RepositoryValidationService.getInstance();
      const service2 = RepositoryValidationService.getInstance();
      
      // Services should be the same instance
      expect(service1).toBe(service2);
      
      // Test private method access through type assertion
      const requestId1 = (service1 as any).generateRequestId();
      const requestId2 = (service1 as any).generateRequestId();
