  ...
  const handleTaskUpdate = useCallback(
    (update: TaskSyncMessage) => {
      ...
      if (repositoryPath && update.repositoryPath !== repositoryPath) {
        log(
          `Ignoring update for ${update.repositoryPath} (monitoring ${repositoryPath})`
        );
        return;
      }
      ...
      try {
        switch (update.event) {
          case 'TASKS_UPDATED':
            ...
            if (currentHandlers.onTasksUpdated) {
              currentHandlers.onTasksUpdated(
            ...
            if (showNotifications) {
              showSuccess(
                ...
                notificationMessages.tasksUpdated ||
                  `Tasks refreshed for ${update.repositoryPath}`,
                { duration: 3000 }
              );
            }
            break;
            ...
            if (showNotifications) {
              showInfo(
                ...
                notificationMessages.repositoryAdded ||
                  `Now monitoring ${update.repositoryPath}`,
                { duration: 4000 }
              );
            }
            break;
            ...
            if (showNotifications) {
              showInfo(
                ...
                notificationMessages.repositoryRemoved ||
                  `Stopped monitoring ${update.repositoryPath}`,
                { duration: 4000 }
              );
            }
            break;
            ...
            if (showNotifications) {
              showError(
                ...
                notificationMessages.tasksError ||
                  `Error updating tasks for ${update.repositoryPath}`,
                { duration: 6000 }
              );
            }
            break;
        ...
        if (showNotifications) {
          showError('Update Handler Error', 'Failed to process task update', {
            duration: 5000,
          });
        }
      }
  ...
  const requestRefresh = useCallback(
    (targetRepository?: string) => {
      ...
      if (target && isConnected()) {
        ...
        log(
          'Cannot request refresh - no repository specified or not connected'
        );
        if (showNotifications) {
          showError(
            'Refresh Failed',
