...
class TaskMasterRouteFactory {
    constructor(taskMasterService, webSocketService) {
        ...
        this.validateTaskCreation = (req) => {
            const errors = [];
            const { title, description, priority, dueDate, estimatedHours, tags, dependencies } = req.body;
            // Title validation
            if (title) {
    ...
    addTaskRoutes(router) {
         ...
         */
        router.post('/tasks', (0, middleware_1.validationMiddleware)({
            bodySchema: {
                required: ['repositoryPath', 'title', 'description', 'priority'],
            },
            customValidators: [
