...
exports.prdController = exports.PrdController = void 0;
...
class PrdController {
    ...
    async analyzePrd(req, res) {
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.debug('PRD content saved to temporary file', { ...context, tempFile }, 'controller');
            }
            catch (error) {
                logger_1.logger.error('Failed to save PRD content to temporary file', context, error, 'controller');
                res.status(500).json({
                    success: false,
            ...
            try {
                ...
                try {
                    ...
                    logger_1.logger.debug('Temporary PRD file cleaned up', { ...context, tempFile }, 'controller');
                }
                catch (error) {
                    logger_1.logger.warn('Failed to clean up temporary PRD file', { ...context, tempFile }, 'controller');
                }
            }
        }
        catch (error) {
            const normalizedError = errorHandler_1.errorHandler.normalizeError(error, { requestId });
            logger_1.logger.error('PRD analysis failed', { requestId }, normalizedError, 'controller');
            res.status(500).json({
                success: false,
    ...
    extractTasksFromOutput(output) {
        try {
            ...
            logger_1.logger.error('Failed to extract tasks from output', { output: output.substring(0, 500) }, error, 'controller');
            return [];
        }
    }
    /**
     ...
     */
    extractComplexityFromOutput(output) {
        try {
            ...
            logger_1.logger.error('Failed to extract complexity from output', { output: output.substring(0, 500) }, error, 'controller');
            return null;
        }
    }
    /**
     ...
     */
    calculateEstimatedEffort(tasks, complexityAnalysis) {
...
exports.PrdController = PrdController;
exports.prdController = new PrdController();
//# sourceMappingURL=prdController.js.map