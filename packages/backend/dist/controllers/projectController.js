...
class ProjectController {
    ...
    async createProject(req, res) {
        ...
        try {
            logger_1.logger.info('Starting project creation', {
                requestId,
                operation: 'create-project',
                timestamp: new Date().toISOString()
            });
            // Validate request body
            const { repositoryId, projectName } = req.body;
            if (!repositoryId) {
                logger_1.logger.warn('Project creation failed: missing repositoryId', {
                    requestId,
            ...
            if (!projectName || typeof projectName !== 'string') {
                logger_1.logger.warn('Project creation failed: invalid projectName', {
                    requestId,
            ...
            if (projectNameTrimmed.length < 2) {
                logger_1.logger.warn('Project creation failed: project name too short', {
                    requestId,
            ...
            if (projectNameTrimmed.length > 50) {
                logger_1.logger.warn('Project creation failed: project name too long', {
                    requestId,
                    projectName: projectNameTrimmed,
            ...
            if (!/^[a-zA-Z0-9\-_\s]+$/.test(projectNameTrimmed)) {
                logger_1.logger.warn('Project creation failed: invalid project name format', {
                    requestId,
                    projectName: projectNameTrimmed,
            ...
            if (!repository) {
                logger_1.logger.warn('Project creation failed: repository not found', {
                    requestId,
                    repositoryId
                });
                res.status(404).json({
            ...
            logger_1.logger.info('Repository found for project creation', {
                requestId,
                repositoryId,
            ...
            logger_1.logger.info('Starting TaskMaster CLI initialization', {
                requestId,
                projectId,
                projectName: projectNameTrimmed,
                repositoryPath: repository.path
            ...
            try {
                const initResult = await taskMasterService_1.taskMasterService.initProject(repository.path);
                if (!initResult.success) {
                    logger_1.logger.error('TaskMaster CLI initialization failed', {
                        requestId,
                        projectId,
                ...
                logger_1.logger.info('TaskMaster CLI initialization completed successfully', {
                    requestId,
                    projectId,
                ...
                logger_1.logger.info('Project creation completed successfully', {
                    requestId,
                    projectId,
                    projectName: projectNameTrimmed,
                    repositoryPath: repository.path,
                ...
                logger_1.logger.error('TaskMaster CLI initialization failed with error', {
                    requestId,
                    projectId,
            ...
            logger_1.logger.error('Project creation failed with unexpected error', {
                requestId,
    ...
    async getProjects(req, res) {
        ...
        try {
            ...
            logger_1.logger.error('Failed to list projects', {
                requestId,
                error: error instanceof Error ? error.message : 'Unknown error',
                stack: error instanceof Error ? error.stack : undefined
            });
    ...
    async getProjectById(req, res) {
        ...
        try {
            logger_1.logger.info('Getting project by ID', {
                requestId,
                projectId,
            ...
            logger_1.logger.error('Failed to get project', {
                requestId,
                projectId,
                error: error instanceof Error ? error.message : 'Unknown error',
                stack: error instanceof Error ? error.stack : undefined
    ...
    async deleteProject(req, res) {
        ...
        const projectId = req.params.id;
        try {
            logger_1.logger.info('Deleting project', {
                requestId,
                projectId,
            ...
            logger_1.logger.error('Failed to delete project', {
                requestId,
                projectId,
                error: error instanceof Error ? error.message : 'Unknown error',
                stack: error instanceof Error ? error.stack : undefined
