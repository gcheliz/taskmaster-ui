...
export class PrdController {
  ...
  async analyzePrd(req: Request, res: Response): Promise<void> {
    ...
    try {
      const {
        ...
        prdContent,
        options = {},
      } = req.body as PrdAnalysisRequest;
      ...
      try {
        ...
        logger.debug(
          'PRD content saved to temporary file',
          { ...context, tempFile },
          'controller'
        );
      } catch (error) {
        logger.error(
          'Failed to save PRD content to temporary file',
          context,
          error as Error,
          'controller'
        );
      ...
      try {
        ...
        const parseResult = await taskMasterService.parsePRD(
          repositoryPath,
          tempFile,
          {
        ...
        const complexityResult = await taskMasterService.analyzeComplexity(
          repositoryPath,
          {
        ...
        if (!complexityResult.success) {
          logger.warn(
            'Complexity analysis failed, proceeding with parsed tasks only',
            context,
            'controller'
          );
        }
        ...
        try {
          ...
          logger.debug(
            'Temporary PRD file cleaned up',
            { ...context, tempFile },
            'controller'
          );
        } catch (error) {
          logger.warn(
            'Failed to clean up temporary PRD file',
            { ...context, tempFile },
            'controller'
          );
        }
  ...
  private extractTasksFromOutput(output: string): unknown[] {
    try {
      ...
      logger.error(
        'Failed to extract tasks from output',
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (trimmed.includes('Complexity:') || trimmed.includes('●')) {
          const complexityMatch = trimmed.match(
            /Task\s+(\d+(?:\.\d+)?)\s*.*?●\s*(\d+)/
          );
          if (complexityMatch) {
      ...
      if (complexityData.tasks.length > 0) {
        ...
        const totalComplexity = complexityData.tasks.reduce(
          (sum: number, task: any) => sum + task.complexity,
          0
        );
      ...
      logger.error(
        'Failed to extract complexity from output',
        { output: output.substring(0, 500) },
        error as Error,
        'controller'
      );
  ...
  private calculateAverageComplexity(complexityAnalysis: any): number {
    if (!complexityAnalysis?.tasks) return 0;
