...
import {
  TaskMasterCommandBuilder,
  taskMasterCommandBuilder,
} from './taskMasterCommandBuilder';
import {
  TaskMasterOutputParser,
  taskMasterOutputParser,
} from './taskMasterOutputParser';
  ...
  implements ITaskMasterService
{
  ...
  async initProject(
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          'Failed to parse init output, using defaults',
          context,
          'service'
        );
  ...
  async getProjectStatus(path: string): Promise<TaskMasterResult<ProjectInfo>> {
    ...
    try {
      ...
      const projectInfo = this.outputParser.extractProjectInfo(
        result.stdout
      ) || {
        name: 'TaskMaster Project',
        path,
  ...
  dispose(): void {
    ...
    try {
      ...
      logger.error(
        'Error disposing TaskMaster service',
        {},
        error as Error,
        'service'
      );
