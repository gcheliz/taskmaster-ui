import {
  ParsedTaskMasterOutput,
  TaskInfo,
  ProjectInfo,
  ITaskMasterOutputParser,
...
class TaskListParsingStrategy implements IParsingStrategy {
  ...
  parse(output: string): ParsedTaskMasterOutput[] {
    ...
    const progressRegex =
      /Progress:\s*█+░*\s*(\d+)%|Tasks Progress:\s*█+░*\s*(\d+)%|(\d+)% complete/gi;
    let progressMatch;
    while ((progressMatch = progressRegex.exec(output)) !== null) {
      const percentage =
        progressMatch[1] || progressMatch[2] || progressMatch[3];
      if (percentage) {
        results.push({
          type: 'info',
  ...
  private parseStatus(statusStr: string): string {
    ...
    if (statusStr.includes('►') || statusStr.includes('in-progress'))
      return 'in-progress';
    if (statusStr.includes('○') || statusStr.includes('pending'))
      return 'pending';
...
class TaskDetailParsingStrategy implements IParsingStrategy {
  canParse(output: string, command: string): boolean {
    return (
      (command.includes('show') && output.includes('Task:')) ||
...
class StatusUpdateParsingStrategy implements IParsingStrategy {
  canParse(output: string, command: string): boolean {
    return (
      command.includes('set-status') && output.includes('Successfully updated')
    );
  }
  ...
  parse(output: string): ParsedTaskMasterOutput[] {
    ...
    const updateRegex =
      /Successfully updated (?:task|subtask) ([\d.]+) status:[\s\S]*?From:\s*([\w-]+)[\s\S]*?To:\s*([\w-]+)/s;
...
class NextTaskParsingStrategy implements IParsingStrategy {
  ...
  parse(output: string): ParsedTaskMasterOutput[] {
    ...
    const nextTaskRegex =
      /Next Task to Work On:\s*#([\d.]+)\s*-\s*(.+?)(?=\s*│|\n|$)/s;
...
class ComplexityAnalysisParsingStrategy implements IParsingStrategy {
  canParse(output: string, command: string): boolean {
    return (
      command.includes('analyze-complexity') &&
  ...
  parse(output: string): ParsedTaskMasterOutput[] {
    ...
    if (avgComplexityMatch || totalTasksMatch) {
      ...
      if (avgComplexityMatch)
        analysisData.averageComplexity = parseFloat(avgComplexityMatch[1]);
      if (totalTasksMatch)
...
class PRDParsingStrategy implements IParsingStrategy {
  ...
  parse(output: string): ParsedTaskMasterOutput[] {
    ...
    const generatedTasksMatch =
      /Generated\s+(\d+)\s+tasks|tasks from PRD/i.exec(output);
    const appendedMatch =
      /Appended\s+(\d+)\s+tasks|appended.*?(\d+).*?tasks/i.exec(output);
...
class ExpansionParsingStrategy implements IParsingStrategy {
  ...
  parse(output: string): ParsedTaskMasterOutput[] {
    ...
    const expandedTaskMatch =
      /Expanded task\s+([\d.]+)\s+into\s+(\d+)\s+subtasks/i.exec(output);
...
class DependencyValidationParsingStrategy implements IParsingStrategy {
  canParse(output: string, command: string): boolean {
    return (
      command.includes('validate-dependencies') &&
  ...
  parse(output: string): ParsedTaskMasterOutput[] {
    ...
    const validationPassedMatch =
      /All dependencies are valid|Validating.*dependencies.*valid/i.exec(
        output
      );
    const issuesFoundMatch =
      /Found\s+(\d+)\s+dependency issues|dependency issues/i.exec(output);
    const circularDepsMatch =
      /Circular dependency detected|circular dependency/i.exec(output);
    ...
    errorLines.forEach(line => {
      ...
      if (cleanLine) {
        ...
        const errorCodeMatch =
          /(?:error|npm error)\s+([A-Z_]+[\d]*):?\s*(.+)|error\s+(TS\d+):\s*(.+)/i.exec(
            cleanLine
          );
...
export class TaskMasterOutputParser implements ITaskMasterOutputParser {
  ...
  extractComplexityAnalysis(output: string): any {
    ...
    const analysisData = parsed.find(
