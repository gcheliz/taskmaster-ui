...
class EnvironmentManager {
  ...
  private loadConfiguration(): EnvironmentConfig {
    ...
    return {
      // Application
      nodeEnv: this.getString('VITE_TASKMASTER_NODE_ENV', mode) as
        | 'development'
        | 'production'
        | 'test',

      // API Configuration
      apiBaseUrl: this.getString(
        'VITE_TASKMASTER_API_BASE_URL',
      ...
      wsUrl: this.getString(
        'VITE_TASKMASTER_WS_URL',
        isDev ? 'ws://localhost:3001' : ''
      ),
      wsReconnectDelay: this.getNumber(
        'VITE_TASKMASTER_WS_RECONNECT_DELAY',
        5000
      ),

      // Security
      enableCSP: this.getBoolean('VITE_TASKMASTER_ENABLE_CSP', true),
      enableRateLimiting: this.getBoolean(
        'VITE_TASKMASTER_ENABLE_RATE_LIMITING',
        true
      ),
      rateLimitMaxAttempts: this.getNumber(
        'VITE_TASKMASTER_RATE_LIMIT_MAX_ATTEMPTS',
        isDev ? 100 : 10
      ),
      rateLimitWindowMs: this.getNumber(
        'VITE_TASKMASTER_RATE_LIMIT_WINDOW_MS',
        15 * 60 * 1000
      ),

      // Development
      enableDebug: this.getBoolean('VITE_TASKMASTER_ENABLE_DEBUG', isDev),
      enableSourceMaps: this.getBoolean(
        'VITE_TASKMASTER_ENABLE_SOURCE_MAPS',
        isDev
      ),
      enableDevTools: this.getBoolean(
        'VITE_TASKMASTER_ENABLE_DEV_TOOLS',
        isDev
      ),

      // Feature Flags
      enableNotifications: this.getBoolean(
        'VITE_TASKMASTER_ENABLE_NOTIFICATIONS',
        true
      ),
      enableThemeToggle: this.getBoolean(
        'VITE_TASKMASTER_ENABLE_THEME_TOGGLE',
        true
      ),
      enableDragDrop: this.getBoolean('VITE_TASKMASTER_ENABLE_DRAG_DROP', true),

      // Limits
      maxRepositories: this.getNumber(
        'VITE_TASKMASTER_MAX_REPOSITORIES',
        isDev ? 50 : 10
      ),
      maxProjectsPerRepo: this.getNumber(
        'VITE_TASKMASTER_MAX_PROJECTS_PER_REPO',
        isDev ? 10 : 5
      ),
      maxTasksPerProject: this.getNumber(
        'VITE_TASKMASTER_MAX_TASKS_PER_PROJECT',
        isDev ? 5000 : 1000
      ),
      autoSaveInterval: this.getNumber(
        'VITE_TASKMASTER_AUTO_SAVE_INTERVAL',
        isDev ? 10000 : 30000
      ),

      // File Upload
      maxFileSize: this.getNumber(
        'VITE_TASKMASTER_MAX_FILE_SIZE',
        10 * 1024 * 1024
      ), // 10MB
      allowedFileTypes: this.getStringArray(
        'VITE_TASKMASTER_ALLOWED_FILE_TYPES',
        ['txt', 'md', 'json']
      ),
      ...
      logLevel: (this.getOptionalString('VITE_TASKMASTER_LOG_LEVEL') ||
        (isDev ? 'debug' : 'info')) as 'debug' | 'info' | 'warn' | 'error',
    };
  }
  ...
  private getStringArray(key: string, defaultValue: string[]): string[] {
    ...
    try {
      ...
      if (this.config?.enableDebug) {
        console.warn(
          `Environment variable ${key} is not a valid array, using default: ${defaultValue}`
        );
      }
      return defaultValue;
    }
  ...
  private validateConfiguration(): void {
    ...
    if (
      this.config.maxFileSize < 1024 ||
      this.config.maxFileSize > 100 * 1024 * 1024
    ) {
    ...
    if (
      this.config.rateLimitMaxAttempts < 1 ||
      this.config.rateLimitMaxAttempts > 10000
    ) {
    ...
    if (
      ...
      this.config.rateLimitWindowMs > 24 * 60 * 60 * 1000
    ) {
...
export const isFeatureEnabled = (
  feature: Parameters<typeof env.isFeatureEnabled>[0]
) => env.isFeatureEnabled(feature);
