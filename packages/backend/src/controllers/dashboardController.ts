...
export class DashboardController {
      ...
      try {
        ...
        res.status(404).json({
          ...
          error: {
            ...
            message:
              'Tasks file not found. Make sure TaskMaster is initialized.',
          },
        });
        return;
      }
  ...
  private calculateTaskMetrics(tasks: Task[]): DashboardData['taskMetrics'] {
    ...
    const complexityCounts = tasks.reduce(
      (acc, task) => {
        ...
        const level =
          complexity >= 8 ? 'high' : complexity >= 5 ? 'medium' : 'low';
  ...
  private generateChartData(
    ...
    const taskCompletionTrend = this.generateCompletionTrend(
      tasks,
      gitActivity
    );
  ...
  private getAverageTimeForComplexity(
    ...
    return (
      complexityMultiplier[complexity as keyof typeof complexityMultiplier] || 4
    );
  }
  ...
  private calculateInsights(
    ...
    const averageTaskComplexity =
      tasks.reduce((sum, task) => sum + (task.complexity || 0), 0) /
      tasks.length;
    ...
    const productivityScore = Math.min(
      ...
      Math.max(
        0,
        taskMetrics.completionRate * 0.6 + averageTaskComplexity * 10 * 0.4
      )
    );
  ...
  private generateRecommendations(
    ...
    if (avgComplexity > 7) {
      recommendations.push(
        'High complexity tasks detected - consider adding more detailed requirements'
      );
    }

    if (taskMetrics.blocked > 0) {
      recommendations.push(
        `${taskMetrics.blocked} blocked tasks need attention`
      );
    }
