...
import { TaskMasterLogger, LogLevel, LogHandler, LogEntry, ConsoleLogHandler } from '../logger';

describe('TaskMasterLogger', () => {
  let logger: TaskMasterLogger;
  let mockHandler: jest.Mocked<LogHandler>;

  beforeEach(() => {
    // Create fresh logger instance for each test
    logger = new (TaskMasterLogger as any)();
    ...
    (logger as any).logHandlers = [mockHandler];
  });
  ...
  describe('Log Level Control', () => {
    it('should respect log level settings', () => {
      ...
      expect(calls[0][0].level).toBe(LogLevel.WARN);
      expect(calls[1][0].level).toBe(LogLevel.ERROR);
    });
    ...
    it('should only log errors when set to ERROR', () => {
      ...
      expect(mockHandler.handle.mock.calls[0][0].level).toBe(LogLevel.ERROR);
    });
  });

  describe('Basic Logging Methods', () => {
    beforeEach(() => {
      logger.setLogLevel(LogLevel.TRACE); // Enable all logs
    });
  ...
  describe('Log Entry Structure', () => {
    ...
    it('should create proper log entry structure', () => {
      ...
      const logEntry: LogEntry = mockHandler.handle.mock.calls[0][0];
...
describe('ConsoleLogHandler', () => {
  ...
  it('should use appropriate console method for each log level', () => {
    ...
    // Test each log level
    handler.handle({ ...baseEntry, level: LogLevel.ERROR });
    expect(consoleSpy.error).toHaveBeenCalled();

    handler.handle({ ...baseEntry, level: LogLevel.WARN });
    expect(consoleSpy.warn).toHaveBeenCalled();

    handler.handle({ ...baseEntry, level: LogLevel.INFO });
    expect(consoleSpy.info).toHaveBeenCalled();

    handler.handle({ ...baseEntry, level: LogLevel.DEBUG });
    expect(consoleSpy.debug).toHaveBeenCalled();

    handler.handle({ ...baseEntry, level: LogLevel.TRACE });
