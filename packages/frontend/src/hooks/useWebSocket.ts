...
export const useTaskCollaboration = (
  ...
  const moveTask = useCallback((taskId: string, toColumn: string, toPosition: number) => {
    if (!isConnected) {
      setError(new Error('Not connected to WebSocket'));
      return;
    }
    ...
    setTasks(prev => prev.map(t => 
      t.id === taskId 
        ? { ...t, status: toColumn as Task['status'], position: toPosition, column: toColumn, updatedAt: new Date().toISOString() }
        : t
    ));
...
export const useUserPresence = (): UseUserPresenceReturn => {
  ...
  useEffect(() => {
    ...
    const unsubscribeCursor = subscribe<{ userId: string; cursor: { x: number; y: number } }>(
      WebSocketEventType.USER_CURSOR_MOVE,
      (payload) => {
        setUserPresence(prev => ({
          ...prev,
          [payload.userId]: {
