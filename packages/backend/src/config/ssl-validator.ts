 ...
 */
function validateCertificateFile(filePath: string, type: 'CA' | 'Certificate' | 'Key'): { valid: boolean; error?: string } {
  if (!filePath) {
...
export function validateSSLConfiguration(): SSLValidationResult {
  ...
  const certValidation = env.SSL_CERT_PATH ? validateCertificateFile(env.SSL_CERT_PATH, 'Certificate') : { valid: true };
  const keyValidation = env.SSL_KEY_PATH ? validateCertificateFile(env.SSL_KEY_PATH, 'Key') : { valid: true };
  const caValidation = env.SSL_CA_PATH ? validateCertificateFile(env.SSL_CA_PATH, 'CA') : { valid: true };
  ...
  const configuration: SSLConfiguration = {
    ...
    cert: env.SSL_CERT_PATH ? fs.readFileSync(env.SSL_CERT_PATH, 'utf8') : undefined,
    key: env.SSL_KEY_PATH ? fs.readFileSync(env.SSL_KEY_PATH, 'utf8') : undefined,
 ...
 * Test SSL connection
 */
export async function testSSLConnection(): Promise<{ success: boolean; error?: string }> {
  try {
 ...
 */
export function checkSSLCertificateExpiration(): { expiring: boolean; daysUntilExpiration?: number; error?: string } {
  const validation = validateSSLConfiguration();
